...
This vignette demonstrates how to use the scINSIGHT package to analyze multiple single-cell gene expression matrices and how to obtain the factorized results from its returned objects.
...
We first introduce how to create an scINSIGHT object with processed data, using a list of simulated gene expression matrices as an example. The simulation process is described in our manuscript. Suppose we have the normalized, filtered, and log2-transformed gene expression matrices saved as a list. This dataset can be downloaded from [here](https://rutgers.box.com/s/atwckw3rvmdels96whkqvjr38vf27tqg).

```{r load count, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
library(scINSIGHT)
# Load the simulated data
counts = readRDS("sim_count.rds")
```

`counts` is a list of six gene expression samples with rows representing 4977 genes.

To create the scINSIGHT object for running the algorithm, we need to provide the condition each sample belongs to through the `condition` parameter in `create_scINSIGHT` function. In this simulation example, the six samples are from three time points (T1, T2, and T3). Therefore, we can initialize the scINSIGHT object using the code below:
...
scobj = create_scINSIGHT(norm.data = counts, condition = sim.condition)
```

### Create an scINSIGHT object with raw data

Now, we introduce how to initialize an scINSIGHT object with raw count data, using a list of two count matrices as an example. If users do not have preferred method, we recommend using Seurat to perform normalization and gene feature selection. Please see the example code below:

```{r load real, eval=FALSE, echo = TRUE}
library(Seurat)
...
counts = lapply(data.list, function(x){
...
```{r set real sample and condition, eval=FALSE, echo = TRUE, message=TRUE, results='hide'}
real.sample = c("sample1", "sample2")
# Name of count list should represent sample names
names(counts) = real.sample
# Name of condition vector should match sample names in count list
real.condition = c("sample1" = "control", "sample2" = "activation")

# Create a scINSIGHT object for real data
scobj = create_scINSIGHT(norm.data = counts, condition = real.condition)
```

## Run the scINSIGHT algorithm

After create scINSIGHT object, we can use the `run_scINSIGHT` function to run the scINSIGHT algorithm, the key parameters include the following:

* *K*. An integer or vector specifying the candidate number(s) of common gene pathways. The default value is `c(5, 7, 9, 11, 13, 15)`. If input a vector, the function will select the optimal value as described in the manuscript.The selected K will be stored in `parameters[["K"]]`.
* *K_j*. Integer value specifying the number of condition-specific gene pathways. We recommend a relative small number (defaults to 2) to facilitate the interpretation of factorization results.
...
scobj = run_scINSIGHT(scobj, K = seq(5,15,2), out.dir = './test_save/',
                      num.cores = 5)
...
After running the algorithm, the results are stored in the scINSIGHT object. We explain the results using the object obtained for the simulate data, which can be downloaded from [here](https://rutgers.box.com/s/atwckw3rvmdels96whkqvjr38vf27tqg).

The selected parameters and scores are saved in the `parameters` slot. In this example, the selected `K` (number of common gene pathways) is 15 and the selected `lda` (regularization parameters) is 0.01.

```{r selected parameters, echo=TRUE}
scobj = readRDS("sim_scobj.rds")
print(scobj@parameters)
```

The `norm.W_2` slot, a list containing the normalized expression levels of common gene pathways in each sample, could be used for visualization and downstream analysis. For example,

```{r tSNE, echo=TRUE, message=FALSE, warning=FALSE}
library(Rtsne)
W2 = Reduce(rbind, scobj@norm.W_2)
set.seed(1)
res_tsne = Rtsne(W2, dims = 2, check_duplicates = FALSE)

# Sample name of each single cell
sample = unlist(lapply(1:length(counts), function(i){
  rep(sim.sample[i], ncol(counts[[i]]))
}))
# Condition of each single cell
condition = unlist(lapply(1:length(counts), function(i){
  rep(sim.condition[i], ncol(counts[[i]]))
}))
# True cell type set in simulation
celltype = unlist(lapply(1:length(counts), function(i){
  lapply(1:ncol(counts[[i]]), function(j){
    toupper(unlist(strsplit(colnames(counts[[i]])[[j]], "[.]"))[[1]])})}))
...
Then, we can obtain the tSNE plots colored by sample, condition, cell type.
...
Finally, we can print the length or dimension of some other slot.

```{r print dim, echo=TRUE, message=TRUE, warning=FALSE, results='hold'}
# V is a matrix whose dimension is K by genes
print(dim(scobj@V))
# H is a list whose length equals the condition number
print(length(scobj@H))
# Dimension of each matrix in H is K_j by genes
print(sapply(scobj@H, dim))
```
